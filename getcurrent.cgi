...
def getCurrent(qid, showall, course="db"):
    C=sqlite3.connect(f"DB/{course}.db")
    if showall == "true":
        showall = True
    if showall == "false":
        showall = False
    onlytoday = not bool(showall)
    if not qid:
        qid=extract(
    ...
    if onlytoday:
        responselist=C.execute("SELECT response FROM responses WHERE question=? AND date(DATE)==date('now')", (qid,)).fetchall()
        comments = C.execute("SELECT response,comment FROM comments WHERE question=? AND date(DATE)==date('now') ORDER BY DATE DESC", (qid,)).fetchall()
    else:
    ...
    if today:
        totaltoday=len(C.execute("SELECT DISTINCT student FROM responses WHERE date(DATE)==date('now')").fetchall())
    else:
        totaltoday=0
...
if __name__ == "__main__":
    f = readfields(["qid", "showall", "course"])
    result = getCurrent(f.qid, f.showall, default(f.course, "db"))
